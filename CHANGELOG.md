...
### Other
 ...
 - The WBI signature's MD5 digest is now formatted with `faster-hex` instead of
...
### New Features
 ...
 - `Ticket` now implements `Serialize` / `Deserialize`, so callers can cache the
   credential locally and skip re-requesting `nav` + `getDanmuInfo` after a restart
   (the token stays usable until the server refuses it).
...
### Fixes
 ...
 - refine the v0.5.1 error check: only a response **without** `data` is treated as an error
   (reported as `FelgensError::ApiError` with `code` and `message`). A non-zero `code` with
   a payload — such as the anonymous `nav` response `-101` that still carries the WBI
   keys — keeps working exactly like before.

## v0.5.1 (2026-09-20)

### Fixes

 - API errors (e.g. `-352` risk control) are now reported as `FelgensError::ApiError` with
   the response `code` and `message`, instead of a confusing
   `missing field data` deserialization error (`nav`, `getDanmuInfo` and `room_init`
   responses are checked for `code != 0` before decoding the payload).
